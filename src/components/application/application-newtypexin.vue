<template>
  <div>
    <div class="navigation">
      <p>
        <span>管理首页&nbsp;>&nbsp;应用&nbsp;>&nbsp;抢单侠&nbsp;>&nbsp;消息管理&nbsp;>&nbsp;站内信</span>
      </p>
    </div>
  ...
  methods: {
    handleRender(num,code) {
      ...
      if (num == 1) {
        ...
        this.http.post(BASE_URL+"/loan/webMailQdx/getWebMailTypeByCode",{data:code}).then(data=>{
          console.log(data)
      ...
      this.$refs[name].validate((valid) => {
        if (valid) {
          ...
          if (this.titlemod == "添加消息类型") {
            postUrl = "/loan/webMailQdx/saveWebMailType"
            list = {
            ...
            postUrl = "/loan/webMailQdx/updateWebMailType"
            list = {
              typeTitle: this.formValidate.mail, //名称
              typeDescription: this.formValidate.city, // 内容,
              typeIcon: this.formValidate.labelUrl,
    ...
    listxuan(){
      this.http
      .post(BASE_URL + "/loan/webMailQdx/queryWebMailTypeList", {})
      .then(resp => {
        console.log(resp);
        if (resp.code == "success") {
    ...
    deleteOk(code){
      // alert(code)
      this.http.post(BASE_URL+"/loan/webMailQdx/delWebMailType",{data:code}).then(data=>{
        console.log(data)
        if(data.code == "success"){
