				...
				<TabPane label="信贷员详情" name="1">
					<div class="mt15">
						...
						<p class="titlebor mt20">
							...
							<span v-if="creditOfficerlist.serviceAmount!='--'&&creditOfficerlist.serviceRate!='--'&&creditOfficerlist.loanHaveType.length>0&&creditOfficerlist.serviceTime!='--'&&creditOfficerlist.loanDeductionSet!='--'" class="ml10">最后一次更新时间：{{creditOfficerlist.loanLastUpdateTime}}</span>
						</p>
						...
						<div class="admission">
							<p v-for="item in operationLog">
								<span>操作时间:&nbsp;{{item.operatorDate}}</span>
								<span class="ml20">操作人:&nbsp;{{item.operator}}</span>
								<span class="ml20">操作内容:&nbsp;{{item.operatorContent}}</span>
							</p>
      ...
      couponflow : [
		...
		methods: {
			...
			dialsub(name) {
				this.$refs[name].validate(valid => {
					if(valid) {
						...
						if(this.formValidatecall.time == "" && this.selnum == 1 && this.modal6) {
							this.$Modal.warning({
								title: "待办时间",
								content: "请选择待办时间"
							});
			...
			dialEecord() {
				...
				this.http
					.post(BASE_URL + "/sale/saleDialRecord/queryPageList4Manager", list)
					.then(data => {
						if(data.code == "success") {
			...
			officerdetails() {
				...
				this.http
					.post(BASE_URL + "/saleLog/saleLog/queryList", list)
					.then(data => {
						if(data.code == "success") {
			...
			headerdata(num) {
				...
				this.http
					.post(
						BASE_URL + "/sale/saleLoanOfficerInfo/pageQueryByCodeCharge",
						list
					)
					.then(data => {
						if(data.code == "success") {
			...
			bars() {
				...
				let list = JSON.parse(utils.getlocal("articlelists"));
				this, http.post(BASE_URL + "/sale/saleLoanOfficerInfo/downUp", {})
			},
		},
