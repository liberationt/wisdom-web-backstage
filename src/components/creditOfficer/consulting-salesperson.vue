						...
						<div class="left clearfix">
							<div class="left">
								<ul class="querysty">
									...
									<li>
										<Select v-model="orderStatus" placeholder="全部" class="mr10" style="width:150px">
	...
	export default {
			...
			return {
				...
				columns7: [{
					...
					},
					{
						...
						align: 'center',
						render: (h, params) => {
							return h('div', {}, params.row.serviceCost + '元')
						}
					},
					{
						title: '操作',
							...
							return h('div', [
								h(
									'Button', {
										...
										on: {
											click: () => {
												this.$router.push({
													path: './consultSalesdetails?saleOrderCode=' + params.row.saleOrderCode + '&num=2'
												})
											}
										}
									},
								...
								h(
									'Button', {
										...
										style: {
											marginRight: '5px',
											display: params.row.orderStatusDetail != "已咨询" ? "none" : "inline-block"
										},
										on: {
											click: () => {
												this.$router.push('/cancellation?orderCode=' + params.row.orderCode + '&num=2')
											}
										}
									},
									'退单'
								...
								h(
									'Button', {
										...
										on: {
											click: () => {
												this.$router.push({
													path: './salesDetails?saleOrderCode=' + params.row.saleOrderCode + '&num=2'
												})
											}
										}
									},
		...
		methods: {
			pageChange(page) {
				...
				let OrderType = utils.getlocal("OrderTypestaff")
				OrderType.pageNum = page
				utils.putlocal('OrderTypestaff', OrderType)
			},
			pagesizechange(page) {
				this.startRow = 1
				this.endRow = page
			...
			consultingorders(startRow) {
				...
				this.http.post(BASE_URL + '/sale/saleOrder/queryRoleOrderList', list)
					.then((resp) => {
