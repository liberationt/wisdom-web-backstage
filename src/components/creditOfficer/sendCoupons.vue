...
<div style="min-width:1070px">
    <div class="navigation">
        <p>
        <span>管理首页&nbsp;>&nbsp;应用>&nbsp;抢单侠>&nbsp;充值活动管理>&nbsp;充值赠送优惠劵</span>
        </p>
      </div>
            ...
            <ul>
              ...
              <li class="clearfix mt15 credit_recharge">
                      ...
                      >
                        ...
                        <Select :disabled="auditing" v-model="item.couponNo" class="left mb5  inputnum" style="width:150px">
...
export default {
    ...
    addnormal (num) {
      ...
      marketingusbmit () {
          ...
          for (let x in this.firstaddnormals[j]) {
            if (x == 'rechargeCount') {
              ...
              if (x!='couponNo') {
                ...
                this.firstaddnormals[j][x][k].couponNo = this.firstaddnormals[j].couponNo
        ...
        this.$Modal.confirm({
          ...
          onOk: () => {        
            ...
            let url = '/loan/rechargeActivity/save-coupon'    
      ...
      inquireabout () {
        ...
        .then((resp) => {
          if (resp.code == 'success') {
            ...
            this.addnormals = resp.data.couponConfig.firstList
            for (let i = 0; i < resp.data.couponConfig.list.length; i++) {
              resp.data.couponConfig.list[i].couponNo = resp.data.couponConfig.list[i].list[0].couponNo
            }
            this.firstaddnormals = resp.data.couponConfig.list
          } else {
            this.$Message.warning(resp.message);                 
          }
        })
      ...
      voucherNumber () {
        this.http.post(BASE_URL + '/loan/marketing/saveViewData',{})
        .then((resp) => {
          if (resp.code == 'success') {
            this.cityList = resp.data.couponKeyList
