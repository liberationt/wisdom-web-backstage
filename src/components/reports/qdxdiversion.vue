<template>
    <div>
		<div class="navigation">
			<p>
				<span>管理首页&nbsp;>&nbsp;报表&nbsp;>&nbsp;抢单侠报表&nbsp;>&nbsp;渠道转化率报表-导流</span>
			</p>
		</div>
...
export default {
	...
	methods:{
		...
		queryList(startRow){
			...
			this.http.post(BASE_URL+"/report/reportQiangDanXiaChannel/queryQiangDanXiaPromotionReport",{
      ...
      untils.exporttable(BASE_URL+'/report/reportQiangDanXiaChannel/exportQiangDanXiaPromotionReport', untils.getlocal("token"), formData, e => {
        this.loading2 = false;
      });
		},
		// 初始化数据
		Initialization(){
			this.http.post(BASE_URL+"/report/reportQiangDanXiaChannel/initPromotionReport",{}).then(data=>{
				if(data.code =='success'){
